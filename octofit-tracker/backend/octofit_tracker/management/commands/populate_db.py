"""Populate the OctoFit database with sample data."""
...
from tracker.models import Activity, Leaderboard, Team, User, Workout
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Activity.objects.create(user=user1, type='Manga Maniacs', duration=60, calories=100, date=today)
        ...
        self.stdout.write(
            self.style.SUCCESS(
                'OctoFit test data created: 3 users, 2 teams, 4 activities, 3 workouts, 3 leaderboard entries.'
            )
        )
